...
import re
import time
from fastapi import FastAPI, HTTPException, Depends, Header, Request, UploadFile, File, Form, Query
...
async def upload_promotion_data(
    file: UploadFile = File(...),
    data_type: str = Form(...),  # search, allsite, scene
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                csv_text = contents.decode("gb2312")
            ...
            for _, row in df.iterrows():
                ...
                for csv_col, db_col in column_mapping.items():
                    if csv_col in row.index and pd.notna(row[csv_col]):
                        ...
                        try:
                            if db_col in ["spend", "sales", "roi", "ctr", "conversion_rate", "cpc"]:
                                data[db_col] = float(value)
                            elif db_col in ["clicks", "impressions"]:
                                data[db_col] = int(float(value))
                            else:
                                data[db_col] = str(value)
                        except:
                            pass
                
                # 创建记录
                from database import UserPromotionData
                promotion_data = UserPromotionData(**data)
                db.add(promotion_data)
                imported_count += 1
...
@app.post("/api/upload-data-json")
async def upload_promotion_data_json(
    request: dict,
    current_user: dict = Depends(get_current_user)
):
    """上传推广数据 CSV (JSON格式)"""
    try:
        ...
        data_type = request.get('data_type', 'search')
        ...
        try:
            for _, row in df.iterrows():
                ...
                column_mapping = {
                    ...
                    "花费": "spend", "spend": "spend", "消耗": "spend",
                    "销售额": "sales", "sales": "sales", "成交额": "sales",
                    "ROI": "roi", "roi": "roi",
                    "点击量": "clicks", "clicks": "clicks",
                    "展现量": "impressions", "impressions": "impressions", "曝光量": "impressions",
                    "点击率": "ctr", "ctr": "ctr",
                    "转化率": "conversion_rate", "conversion_rate": "conversion_rate",
                    "平均点击单价": "cpc", "cpc": "cpc", "点击单价": "cpc",
                }
...
async def get_user_promotion_data(
    data_type: str = Query(None),  # search, allsite, scene
    limit: int = Query(100),
    current_user: dict = Depends(get_current_user)
):
    """获取用户的推广数据"""
    ...
    try:
        ...
        query = db.query(UserPromotionData).filter(
            UserPromotionData.user_id == current_user.id
        )
        
        if data_type:
            query = query.filter(UserPromotionData.data_type == data_type)
        
        data = query.order_by(desc(UserPromotionData.created_at)).limit(limit).all()
        
        # 计算汇总数据
        # 计算汇总数据
        total_spend = sum(d.spend or 0 for d in data)
        total_sales = sum(d.sales or 0 for d in data)
...
# 市场数据采集相关
from intelligence.data_collector import MarketDataCollector
from intelligence.proxy_pool import ProxyPool
from intelligence.collaborative_filter import CollaborativeFilter
from intelligence.market_insights import MarketInsights
...
@app.get("/api/market/category-trends")
async def get_category_trends(category: str = "数码配件"):
    """获取类目趋势数据"""
    try:
        data = market_collector.get_category_trends(category)
        return {
            "success": True,
            "data": data
        }
    except Exception as e:
        logger.error(f"获取类目趋势失败: {str(e)}")
        return {
            "success": False,
            "error": str(e)
        }

@app.get("/api/market/competitor-prices")
async def get_competitor_prices(product: str = "手机壳"):
    """获取竞品价格数据"""
    try:
        data = market_collector.get_competitor_prices(product)
        return {
            "success": True,
            "data": data
        }
    except Exception as e:
        logger.error(f"获取竞品价格失败: {str(e)}")
        return {
            "success": False,
            "error": str(e)
        }

@app.get("/api/market/hot-keywords")
async def get_hot_keywords(category: str = "数码配件"):
    """获取热门关键词"""
    try:
        data = market_collector.get_hot_keywords(category)
        return {
            "success": True,
            "data": data
        }
...
async def get_market_insights(category: str = "数码配件"):
    """获取市场洞察报告"""
    try:
        data = market_insights.generate_report(category)
        return {
            "success": True,
            "data": data
        }
...
async def get_user_recommendations(user_id: str, top_n: int = 10):
    """获取协同过滤推荐"""
    try:
        recommendations = collaborative_filter.recommend(user_id, top_n)
        user_stats = collaborative_filter.get_user_stats(user_id)
        return {
            "success": True,
            "data": {
...
async def add_proxy(request: dict):
    """添加代理"""
    try:
        proxy_url = request.get("proxy_url")
        proxy_type = request.get("proxy_type", "http")
        
        if not proxy_url:
            return {"success": False, "error": "缺少proxy_url参数"}
        
        result = proxy_pool.add_proxy(proxy_url, proxy_type)
        return result
    except Exception as e:
...
async def check_proxy_health(proxy_id: int = None):
    """代理健康检查"""
    try:
        result = proxy_pool.check_proxy_health(proxy_id)
        return result
    except Exception as e:
        logger.error(f"代理健康检查失败: {str(e)}")
        return {
            "success": False,
            "error": str(e)
        }
...
async def ai_analyze(request: Request, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        data_type = body.get("data_type", "search")  # search, allsite, scene
        ...
        try:
            ...
            # 获取用户最近的推广数据
            user_data = db.query(UserPromotionData).filter(
                UserPromotionData.user_id == current_user.id,
                UserPromotionData.data_type == data_type
            ).order_by(desc(UserPromotionData.created_at)).limit(100).all()
            
            if not user_data:
                return {
                    "success": False,
                    "message": "暂无数据，请先上传推广数据"
                }
            
            # 转换为分析格式
            data = {
                "total_spend": sum(d.spend for d in user_data),
                "total_sales": sum(d.sales for d in user_data),
                "avg_roi": sum(d.sales for d in user_data) / sum(d.spend for d in user_data) if sum(d.spend for d in user_data) > 0 else 0,
                "items": [
                    {
                        "keyword": d.keyword or d.product_name,
                        "spend": d.spend,
                        "sales": d.sales,
                        "roi": d.roi,
                        "clicks": d.clicks,
...
async def update_notification_settings(
    ...
    db = SessionLocal()
    try:
        from database import NotificationSettings
        ...
        if "notify_risks" in request:
            settings.notify_risks = request["notify_risks"]
        if "notify_opportunities" in request:
            settings.notify_opportunities = request["notify_opportunities"]
        if "notify_reports" in request:
            settings.notify_reports = request["notify_reports"]
...
async def get_ab_tests(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        ).order_by(desc(ABTest.created_at)).all()
            ...
            "data": [
                {
                    ...
                    "variant_a": {
                        ...
                        "spend": t.variant_a_spend,
                        "sales": t.variant_a_sales,
                        "roi": t.variant_a_roi
                    },
                    "variant_b": {
                        "name": t.variant_b_name,
                        "spend": t.variant_b_spend,
                        "sales": t.variant_b_sales,
                        "roi": t.variant_b_roi
                    },
...
async def create_ab_test(request: dict, current_user: dict = Depends(get_current_user)):
    db = SessionLocal()
    try:
        from database import ABTest
        ...
        test = ABTest(
            ...
            variant_a_name=request.get("variant_a_name", "对照组"),
            variant_b_name=request.get("variant_b_name", "实验组")
        )
...
@app.post("/api/ab-tests/{test_id}/update-data")
async def update_ab_test_data(
    test_id: int,
    request: dict,
    current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        # 更新数据
        if "variant_a" in request:
            test.variant_a_spend = request["variant_a"].get("spend", 0)
            test.variant_a_sales = request["variant_a"].get("sales", 0)
            test.variant_a_roi = request["variant_a"].get("roi", 0)
        
        if "variant_b" in request:
            test.variant_b_spend = request["variant_b"].get("spend", 0)
            test.variant_b_sales = request["variant_b"].get("sales", 0)
            test.variant_b_roi = request["variant_b"].get("roi", 0)
