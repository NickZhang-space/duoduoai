...
def build_diagnosis_prompt(data: Dict[str, Any]) -> str:
    ...
    prompt = f"""
你是拼多多推广专家，正在诊断一个商家的推广账户。

数据概览：
{json.dumps(data, ensure_ascii=False, indent=2)}

**重要：你必须找出至少3个具体问题，不能说"未发现明显问题"。**
...
{{
  "issues": [
    {{
      "title": "问题标题（简短有力）",
      "description": "详细描述问题和数据支撑",
      "severity": "high/medium/low",
      "impact": "预计影响（如：每日浪费XX元）"
    }}
  ],
  "summary": "整体诊断总结（2-3句话）"
}}

**必须找出3个具体问题，基于数据给出可执行的建议。**
"""
    return prompt
...
def build_growth_prompt(data: Dict[str, Any]) -> str:
    ...
    prompt = f"""
你是拼多多推广专家，正在为商家寻找增长机会。

数据概览：
{json.dumps(data, ensure_ascii=False, indent=2)}

**重要：你必须找出至少3个具体的增长机会，不能说"暂无机会"。**

请从以下角度寻找机会：
1. 高ROI关键词（ROI>2.0）- 可以加大投放
2. 高点击率低转化词 - 优化详情页可提升
3. 低成本高转化词 - 可以提高出价抢排名
4. 时段机会 - 某些时段转化率特别高
5. 预算机会 - 好词预算不足，可以加预算
...
{{
  "opportunities": [
    {{
      "title": "机会标题（简短有力）",
      "description": "详细描述机会和数据支撑",
      "potential": "预期收益（如：预计ROI提升至X.X）",
      "action": "具体行动建议"
    }}
  ],
  "summary": "整体机会总结（2-3句话）"
}}

**必须找出3个具体机会，基于数据给出可执行的建议。**
"""
    return prompt
